...
def create_account():
    ...
    # Danh sách tên
    names = {
        'first': ['DAVID', 'BIP', 'DEPI', 'BIPEEE', 'NOPO', 'NOEW'],
        'last': ['POEN', 'MAI', 'Minh', 'QUE', 'NANHO', 'ENU', 'MU'],
        'mid': ['OMOO', 'ENU', 'OM', 'NO', 'LENO', 'LEMO', 'OMNO']
    }

    # Tạo tên ngẫu nhiên
    random_first_name = random.choice(names['first'])
    random_name = f"{random.choice(names['mid'])} {random.choice(names['last'])}"
    password = f'Manhhregvip{random.randint(0, 9999999)}?#@'
