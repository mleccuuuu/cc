...
class Facebook_Api (object):
	def __init__(self, cookie):
		...
		self.headers = {'authority': 'www.facebook.com','cache-control': 'max-age=0','sec-ch-ua': '"Google Chrome";v="93", " Not;A Brand";v="99", "Chromium";v="93"','sec-ch-ua-mobile': '?0','sec-ch-ua-platform': '"Windows"','upgrade-insecure-requests': '1','origin': 'https://www.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': 'Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.82 Safari/537.36','accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'navigate','sec-fetch-user': '?1','sec-fetch-dest': 'document','referer': 'https://www.facebook.com/','accept-language': 'en-US,en;q=0.9','cookie': cookie}
	
	def get_thongtin(self):
		try:
		#if True:
			#url = requests.get('https://www.facebook.com/profile.php',headers=self.headers).url
			home = requests.get('https://www.facebook.com/profile.php',headers=self.headers).text
	...
	def reac_cmt(self, id, type_react):
		access = ""
		url = requests.get("https://www.facebook.com/"+id, headers=self.headers, proxies="").url
		if id in url:
			return False
		...
		while True:
			if id in access:
				...
				access = requests.get("https://www.facebook.com/reactions/picker/?"+find_cmt, headers=self.headers).text
				ufi = access.split('/ufi/reaction/?')
				hoan_thanh = requests.get("https://www.facebook.com/ufi/reaction/?"+ufi[index].split('"')[0].replace("amp;", ""), headers=self.headers).text
				return True
			else:
				if "/comment/replies/" in url:
					xemthemcmt = access.split('/comment/replies/?')[1].split('"')[0].replace("amp;", "")
					access = requests.get("https://www.facebook.com/comment/replies/?"+xemthemcmt, headers=self.headers).text
				else:
					xemthemcmt = access.split('/story.php?')[1].split('</a></div></div>')[0].replace("amp;", "").split('"')[0]
					access = requests.get("https://www.facebook.com/story.php?"+xemthemcmt, headers=self.headers).text
	...
	def like(self, id, type):
		...
		try:
			link = 'https://www.facebook.com/reactions/picker/?ft_id='+uid
			...
			reac = requests.get('https://www.facebook.com'+cx, headers = headers).text
			#print(reac)
			return True
		except:
			return False 
	...
	def comment (self, id, msg):
		try:
			...
			if cmt == []: return False
			hoan_thanh = requests.post("https://www.facebook.com%s"%cmt[0].replace('"', "").replace("amp;", ""), headers=self.headers, data={"fb_dtsg": self.fb_dtsg,"jazoest": self.jazoest, "comment_text": msg}).text
			return hoan_thanh
		except:
			return False
	...
	def share(self, id, nd="NDK"):
		try:
			...
			ac = requests.get('https://www.facebook.com/composer/mbasic/?c_src=share'+truycapshare, headers=self.headers).text
           ...
           "fb_dtsg": fb,
			...
			hoan_thanh = requests.post('https://www.facebook.com/composer/mbasic/?csid='+share, headers=self.headers, data=data).text
			return True
		except:
			return False 
