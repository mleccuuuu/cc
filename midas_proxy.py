...
if response.status_code == 200:
    ...
    print("Lỗi khi tải:", response.status_code)
...
class MidasRWA:
    def __init__(self) -> None:
        ...
        self.source = 'ref_c365f430-5a6f-48e3-a0ce-5fa9a35946af'
        self.proxies = []
        self.proxy_index = 0
    ...
    def welcome(self):
        print(
            f"""
            {Fore.YELLOW + Style.BRIGHT}Tool Midas auto
            """
        )
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 400:
                    return self.print_message("Task", Fore.WHITE, f"{title} | {task_id}"
    ...
    def process_query(self, query: str, user_id: str, use_proxy: bool, account_index: int):
        ...
        while token is None:
            ...
            if tasks:
                filtered_tasks = [
                        ...
                        if task["state"] != "COMPLETED" and task["id"] not in skip_tasks
                    ]
                self.print_message("Task Lists", Fore.GREEN, "Available "
                    f"{Fore.WHITE+Style.BRIGHT}{len(filtered_tasks)} Tasks{Style.RESET_ALL}"
                )
                            ...
                            if start and start.get('state') == "CLAIMABLE":
                                ...
                                if delay is not None:
                                    ...
                                    print(
                                            ...
                                            f"{Fore.YELLOW + Style.BRIGHT} {delay} {Style.RESET_ALL}"
    ...
    def main(self):
        try:
            ...
            while True:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=int(config["max_thread"])) as executor:
                    futures = {executor.submit(self.process_query, query, self.load_account_data(query)[0], use_proxy, index + 1): index for index, query in enumerate(queries) if query}

                    # Wait for all futures to complete
                    for future in concurrent.futures.as_completed(futures):
                        index = futures[future]  # Lấy chỉ số của query
                        try:
                            result = future.result()  # Get the result of the future
                        except Exception as e:
                            self.log(f"{Fore.RED + Style.BRIGHT}An error occurred for query {queries[index]}: {e}{Style.RESET_ALL}")

                self.log(f"{Fore.CYAN + Style.BRIGHT}={Style.RESET_ALL}"*50)

                seconds = int(config["time_sleep_minute"]) * 60 * 60
                while seconds > 0:
